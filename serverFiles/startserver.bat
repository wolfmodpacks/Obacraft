...
IF NOT EXIST "%cd%\serverstarter-2.3.1.jar" (
   ...
   ECHO Credits to AOF TEAM for the scripts to start this server
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/Ocraftyone/ServerStarter-CFCorePatch/releases/download/v2.3.1/serverstarter-2.3.1.jar "%cd%\serverstarter-2.3.1.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
