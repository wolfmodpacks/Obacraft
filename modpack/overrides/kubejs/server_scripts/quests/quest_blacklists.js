...
function isBlacklisted(id, player, blacklist) {
  ...
  function handleBlacklistedItem(event, blacklistedItem) {
    event.cancel();
    event.player.displayClientMessage(blacklistedItem.message, true);
  
    const entity = event.entity;
    const item = blacklistedItem.id;
    let count = 0;
  ...
  ItemEvents.dropped((event) => blackListCheck(event.item, event));
  ...
  PlayerEvents.inventoryChanged((event) => blackListCheck(event.item, event));